...
public abstract class AbstractDraggable implements Draggable, QuickImports, QuickLogger {
    ...
    protected void renderShape(net.minecraft.client.util.math.MatrixStack matrix, float x, float y, float w, float h, ru.cookiedlc.api.system.shape.ShapeProperties props) {
        if (Hud.getInstance() != null && Hud.getInstance().useBlur.isValue()) {
            blur.render(props);
        } else {
            rectangle.render(props);
        }
    }
