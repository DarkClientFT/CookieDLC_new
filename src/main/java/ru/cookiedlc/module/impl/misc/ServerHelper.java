...
import ru.cookiedlc.ui.hud.render.CoolDowns;
...
import ru.cookiedlc.module.impl.render.ProjectilePrediction;
...
public class ServerHelper extends Module {
    ...
    StopWatch useItemWatch = new StopWatch();
    ...
    @NonFinal
    UseItemState useItemState = UseItemState.NONE;
    @NonFinal
    int savedSlot = -1;
    @NonFinal
    int targetSlot = -1;
    @NonFinal
    boolean itemFromInventory = false;
    @NonFinal
    Slot inventorySlot = null;
    @NonFinal
    Item currentItem = null;

    BindSetting explosiveTrapBind = new BindSetting("Explosive Trap", "Explosive Trap Key");
    BindSetting defaultTrapBind = new BindSetting("Default Trap", "Default Trap Key");
    BindSetting stunBind = new BindSetting("Stun", "Stun Key");
    BindSetting fireChargeBind = new BindSetting("Explosive Thing", "Explosive Thing Key");
    BindSetting snowballHolyBind = new BindSetting("SnowBall", "SnowBall Key");
    ...
    BooleanSetting autoRepairSetting = new BooleanSetting("Auto Repair", "Auto repairs armor with experience bubble at low durability")
            .setValue(true);
    GroupSetting hwHelper = new GroupSetting("HolyWorld Items", "Items for HolyWorld server")
            .settings(explosiveTrapBind, defaultTrapBind, stunBind, fireChargeBind, snowballHolyBind, autoLootSetting, autoShulkerSetting, autoRepairSetting)
            .setValue(true);

    BindSetting antiFlyBind = new BindSetting("Anti Fly", "Anti Fly Key");
    BindSetting expScrollBind = new BindSetting("Experience Scroll", "Experience Scroll Key");

    GroupSetting rwHelper = new GroupSetting("RW Helper", "Items for ReallyWorld server")
            .settings(antiFlyBind, expScrollBind)
            .setValue(true);

    BindSetting godsAuraBind = new BindSetting("Gods Aura", "God's Aura Key");
    BindSetting trapCopyBind = new BindSetting("Trap", "Trap Key");
    BindSetting plastBind = new BindSetting("Plast", "Plast Key");
    BindSetting clearDustBind = new BindSetting("Clear Dust", "Clear Dust Key");
    BindSetting fireTornadoBind = new BindSetting("Fire Tornado", "Fire Tornado Key");
    BindSetting disorientationBind = new BindSetting("Disorientation", "Disorientation Key");
    BindSetting snowballCopyBind = new BindSetting("SnowBall Copy", "SnowBall Key");
    ...
    BooleanSetting consumablesSetting = new BooleanSetting("Consumables Timer", "Displays the time until the consumable runs out")
            .setValue(true);
    GroupSetting ftHelper = new GroupSetting("FT Helper", "Items for FunTime server")
            .settings(godsAuraBind, trapCopyBind, plastBind, clearDustBind, fireTornadoBind, disorientationBind, snowballCopyBind, autoPointSetting,consumablesSetting)
            .setValue(true);


    public void initialize() {
        keyBindings.add(new KeyBind(Items.PRISMARINE_SHARD, explosiveTrapBind, 0, hwHelper));
        keyBindings.add(new KeyBind(Items.POPPED_CHORUS_FRUIT, defaultTrapBind, 0, hwHelper));
        keyBindings.add(new KeyBind(Items.NETHER_STAR, stunBind, 0, hwHelper));
        keyBindings.add(new KeyBind(Items.FIRE_CHARGE, fireChargeBind, 0, hwHelper));
        keyBindings.add(new KeyBind(Items.SNOWBALL, snowballHolyBind, 0, hwHelper));

        keyBindings.add(new KeyBind(Items.FIREWORK_STAR, antiFlyBind, 0, rwHelper));
        keyBindings.add(new KeyBind(Items.FLOWER_BANNER_PATTERN, expScrollBind, 0, rwHelper));

        keyBindings.add(new KeyBind(Items.PHANTOM_MEMBRANE, godsAuraBind, 0, ftHelper));
        keyBindings.add(new KeyBind(Items.NETHERITE_SCRAP, trapCopyBind, 0, ftHelper));
        keyBindings.add(new KeyBind(Items.DRIED_KELP, plastBind, 0, ftHelper));
        keyBindings.add(new KeyBind(Items.SUGAR, clearDustBind, 0, ftHelper));
        keyBindings.add(new KeyBind(Items.FIRE_CHARGE, fireTornadoBind, 0, ftHelper));
        keyBindings.add(new KeyBind(Items.ENDER_EYE, disorientationBind, 0, ftHelper));
        keyBindings.add(new KeyBind(Items.SNOWBALL, snowballCopyBind, 0, ftHelper));

        setup(ftHelper, hwHelper, rwHelper);
    }
    ...
    public void onKey(KeyEvent e) {
        if (useItemState != UseItemState.NONE) return;

        for (KeyBind bind : keyBindings) {
            if (e.isKeyReleased(bind.setting.getKey()) && bind.group.isValue() && validDistance(bind.distance)) {
                startUseItem(bind.item);
                break;
            }
        }
    }
    ...
    public void onTick(TickEvent e) {
        if (useItemState != UseItemState.NONE) {
            processUseItem();
        }
    }
    ...
    private void startUseItem(Item item) {
        ...
        useItemState = UseItemState.SWITCH_SLOT;
        useItemWatch.reset();
    }
    ...
    private void processUseItem() {
        switch (useItemState) {
            case SWITCH_SLOT -> {
                ...
                useItemState = UseItemState.WAIT;
            }
            case WAIT -> {
                if (useItemWatch.every(50)) {
                    useItemState = UseItemState.USE;
                }
            }
            case USE -> {
                rightClick();
                useItemState = UseItemState.SWAP_BACK;
                useItemWatch.reset();
            }
            case SWAP_BACK -> {
                if (useItemWatch.every(50)) {
                    if (itemFromInventory && inventorySlot != null) {
    ...
    private void resetUseItemState() {
        useItemState = UseItemState.NONE;
        savedSlot = -1;
        targetSlot = -1;
        itemFromInventory = false;
        inventorySlot = null;
    ...
    public void onPacket(PacketEvent e) {
        if (!PlayerIntersectionUtil.nullCheck()) switch (e.getPacket()) {
            case ItemPickupAnimationS2CPacket item when autoShulkerSetting.isValue() && hwHelper.isValue() && item.getCollectorEntityId() == mc.player.getId() && mc.world.getEntityById(item.getEntityId()) instanceof ItemEntity entity -> {
            ...
            case ChunkDeltaUpdateS2CPacket chunkDelta when consumablesSetting.isValue() && ftHelper.isValue() -> {
            ...
            case GameMessageS2CPacket gameMessage when autoPointSetting.isValue() && ftHelper.isValue() -> {
    ...
    public void onRotationUpdate(RotationUpdateEvent e) {
        if (e.getType() == EventType.PRE) {
            if (autoRepairSetting.isValue() && hwHelper.isValue() && StreamSupport.stream(mc.player.getArmorItems().spliterator(), false).anyMatch(stack -> {
            ...
            if (autoLootSetting.isValue() && hwHelper.isValue()) {
                PlayerIntersectionUtil.streamEntities().filter(MerchantEntity.class::isInstance).map(MerchantEntity.class::cast)
                        .filter(m -> m.hasStackEquipped(EquipmentSlot.MAINHAND) || m.hasStackEquipped(EquipmentSlot.OFFHAND)).findFirst().ifPresent(merchant -> {
                            Vec3d attackVector = pointFinder.computeVector(merchant, 6, RotationController.INSTANCE.getRotation(), new LinearMode().randomValue(), true).getLeft();
    ...
    public void onWorldRender(WorldRenderEvent e) {
        ...
        keyBindings.stream().filter(bind -> bind.group.isValue() && PlayerIntersectionUtil.isKey(bind.setting) && PlayerInventoryUtil.getSlot(bind.item) != null).forEach(bind -> {
    ...
    public void onDraw(DrawEvent e) {
        ...
        structures.forEach(cons -> {
            ...
            if (ProjectionUtil.canSee(cons.vec) && cons.anarchy == ServerUtil.getAnarchy() && ServerUtil.getWorldType().equals(cons.world)) {
                rectangle.render(ShapeProperties.create(matrix, posX - padding, posY - padding, width + padding * 2, 10)
    ...
    private void draw(MatrixStack matrix, FontRenderer font, List<String> list, Vec3d vec3d) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            rectangle.render(ShapeProperties.create(matrix, posX - 2, vec3d.y - 2 + offsetY, width + 2 * 2, 10)
    ...
    private enum UseItemState {
        NONE, SWITCH_SLOT, WAIT, USE, SWAP_BACK
    }

    public record KeyBind(Item item, BindSetting setting, float distance, GroupSetting group) {
    }
