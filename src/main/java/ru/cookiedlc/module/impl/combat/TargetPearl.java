...
import ru.cookiedlc.event.events.player.PostMotionEvent;
...
public class TargetPearl extends Module {
    ...
    public void onRotationUpdate(RotationUpdateEvent e) {
        if (e.getType() == EventType.PRE) {
            LivingEntity target = KillAura.getInstance().getLastTarget();
            ...
            PlayerIntersectionUtil.streamEntities().filter(EnderPearlEntity.class::isInstance).map(EnderPearlEntity.class::cast)
                    ...
                    .ifPresent(pearl -> {
                        ...
                        IntStream.range(-89, 89).mapToObj(pitch -> new Angle(yaw, pitch)).filter(angle -> {
                            ...
                            RotationController.INSTANCE.rotateTo(new Angle.VecRotation(angle, angle.toVector()), mc.player, 1, new RotationConfig(new WatchAIMode(),true,true), TaskPriority.HIGH_IMPORTANCE_3, this);
